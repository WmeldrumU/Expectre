...
{
	static std::vector<const char*> validation_layers = { "VK_LAYER_KHRONOS_validation" };

	auto instance_extensions = ToolsVk::get_required_instance_extensions(_is_debug_build);
...
void RenderContextVk::create_device()
{
	...
	{
		vkGetPhysicalDeviceSurfaceSupportKHR(m_physical_device, i,
	...
	VK_CHECK_RESULT(vkCreateDevice(m_physical_device, &device_create_info, nullptr, &m_device));
...
const VkDevice& RenderContextVk::get_device() {
	return m_device;
}

const VkPhysicalDevice& RenderContextVk::get_phys_device() {
